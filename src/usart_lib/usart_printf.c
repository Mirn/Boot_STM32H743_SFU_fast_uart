...
const char src_ver_usart_printf[] = __DATE__"\t"__TIME__"\t" __FILE__"\r";
 ...
 */
static inline signed int PutUnsignedInt(
    char *pStr,
    char fill,
    signed int width,
    unsigned int value)
...
{
    return vfprintf(NULL, pFormat, ap); //(stdout, pFormat, ap);
}
...
signed int puts(const char *pStr)
{
    signed int i = fputs(pStr, NULL); //stdout);
    fputc('\n', NULL); //stdout);
